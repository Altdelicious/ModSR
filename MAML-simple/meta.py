...
class Learner(nn.Module):
    ...
    then backpropped on theta network, which should be done on metalaerner class.
    For learner class, it will be responsible for update for several steps on meta-train set and return with the loss on
    meta-test set.
    """

    def __init__(self, net_cls, weights_file, device0, device1):
        """
        ...
        self.net_pi = net_cls()
        self.net_pi.to(device1)
        self.load_pre_weights(weights_file)
        ...
        self.optimizer = optim.Adam(params=filter(lambda p: p.requires_grad, self.net_pi.parameters()), lr=0.000001)
        #self.optimizer = optim.Adam(self.net_pi.parameters(), lr=0.01)
        
    def load_pre_weights(self, weights_file):
        """
        ...
        state_dict = self.net_pi.state_dict()
    ...
    def forward(self, support_x, support_y, query_x, query_y, num_updates):
        ...
        #print(self.net_pi.dummy_param.device)
        #print(query_x.get_device(), query_y.get_device())
        ...
        #print(grads_pi[0], grads_pi[1])
        #print(loss.item())

        #return loss, grads_pi, acc
        return loss, grads_pi
...
class MetaLearner(nn.Module):
    ...
    def __init__(self, net_cls, weights_file, meta_batchsz, beta, num_updates, device0, device1, output_file):
        """
        ...
        self.learner = Learner(net_cls, weights_file, device0, device1)
        # the optimizer is to update theta parameters, not theta_pi parameters.
        self.optimizer = optim.Adam(self.learner.params(), lr=beta)
        miles = [1000, 2000, 3000, 4000]
        self.scheduler = optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=miles, gamma=0.5, last_epoch=-1, verbose=False)
    ...
    def write_grads(self, dummy_loss, sum_grads_pi):
        ...
        for i, v in enumerate(self.learner.params()):
            def closure():
                ii = i
    ...
    def forward(self, support_x, support_y, query_x, query_y):
        ...
        for i in range(meta_batchsz):
            _, grad_pi = self.learner(support_x[i], support_y[i], query_x[i], query_y[i], self.num_updates)
            ...
            if sum_grads_pi is None:
                ...
                sum_grads_pi = [torch.add(m, n) for m, n in zip(sum_grads_pi, grad_pi)]
        ...
        #torch.save(self.learner.net.state_dict(), 'vdsr_aniso_5000.pth')
        torch.save(self.learner.net.state_dict(), self.output_file)
    ...
    def pred(self, support_x, support_y, query_x, query_y):
        ...
        for i in range(meta_batchsz):
            _, _ = self.learner(support_x[i], support_y[i], query_x[i], query_y[i], self.num_updates)
            #accs.append(episode_acc)
